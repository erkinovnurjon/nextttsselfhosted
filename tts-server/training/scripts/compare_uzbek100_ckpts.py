# uzbek100 checkpoint'larini solishtiradi (ckpts/uzbek100/model_*.pt + model_last)
# + uzbek70/model_last va ayol/model_12000 baseline. 6 standart gap, whisper ASR WER.
# Audio: output/compare_uzbek100/ + public/uzbek100_ckpt/ (sahifa public/uzbek100_ckpt.html).
#   cd tts-server
#   KMP_DUPLICATE_LIB_OK=TRUE WHISPER_DEVICE=cpu PYTHONUTF8=1 .venv-f5/Scripts/python.exe training/scripts/compare_uzbek100_ckpts.py
...
VOICES = ROOT / "voices"
OUT = ROOT / "output" / "compare_uzbek100"
OUT.mkdir(parents=True, exist_ok=True)
PUB = ROOT.parent / "public" / "uzbek100_ckpt"
...
def discover_models():
    ...
    steps.sort()
    for s, p in steps:
        models.append((f"uz100_{s//1000}k", p))
    if (d / "model_last.pt").exists():
        models.append(("uz100_last", d / "model_last.pt"))
    if (CK / "uzbek70" / "model_last.pt").exists():
        models.append(("uzbek70_base", CK / "uzbek70" / "model_last.pt"))
...
for name, _ in MODELS:
    ...
    r = results[name]
    cells = "".join(
        f'<td><audio controls preload="none" src="uzbek100_ckpt/{name}_{i}.wav"></audio>'
...
<title>uzbek100 checkpoint solishtiruv</title>
...
</head><body><h2>uzbek100 (to'liq ISSAI) — checkpoint solishtiruv (seed 1234, nfe 48)</h2>
<table><tr><th>Checkpoint</th>{heads}</tr>{''.join(rows)}</table></body></html>"""
(ROOT.parent / "public" / "uzbek100_ckpt.html").write_text(html, encoding="utf-8")
print(f"Audio: {OUT}\nSahifa: public/uzbek100_ckpt.html")
